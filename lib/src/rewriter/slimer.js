...
class Rewriter {
  ...
  rewrite(iterator) {
    // we will call next() as we loop along...
    this.iterator = iterator;

    this.startCodeBlock();

    // kick off the loop with the first item
    return this._iterate(iterator[Symbol.iterator]().next());
  }

  _iterate(item) {
    // base case is that we are done
    if (item.done) return this._close();
    // if this is not an import, just handle it normally
    if (item.value.tokens[0][0] !== 'IMPORT') {
      this.handleItem(item.value);
      // recurse along...
      return this._iterate(this.iterator[Symbol.iterator]().next());
    } else {
      // handle the imports by lexing the lines and assembling an item...
    }
  }
